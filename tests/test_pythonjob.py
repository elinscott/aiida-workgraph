...
def keep_as_node_is_available() -> bool:
    """True when the installed aiida-pythonjob can be told to keep an input as a node."""
    import inspect
...
def test_node_arrival_paths_names_every_node_typed_leaf():
    """The paths a PyFunction is told to keep, read off the spec alone."""
    from aiida import orm
    from aiida_workgraph.tasks.pythonjob_tasks import node_arrival_paths
    from node_graph.input_model import spec_from_model
    ...
    paths = node_arrival_paths(spec_from_model(Outer))

    assert sorted(paths) == ['nested.structure', 'payload']
    # A spec no model owns carries no mark, so nothing is named.
    assert node_arrival_paths(namespace(x=int, y=str)) == []


@pytest.mark.skipif(not keep_as_node_is_available(), reason='needs aiida-pythonjob with `keep_as_node`')
def test_a_model_declared_node_reaches_the_body_as_the_node():
    """A field the model can only satisfy with a node is handed the node."""
    from aiida import orm
    from pydantic import BaseModel, ConfigDict
    ...
    @task(input_model=Inputs, outputs=['seen'])
    ...
    wg = WorkGraph('model_declared_node')
    node = wg.add_task(reads_a_structure, name='leaf', structure=structure)
    wg.run()
    assert node.outputs.seen.value.value == 'StructureData'

    # A task with no model is untouched: its socket still arrives deserialized.
    wg = WorkGraph('annotated_node')
